...
## 소유자 검수 완료 선언 및 스토어 등록 준비 — 2026-09-19

- 소유자 요청 원문: “콘텐츠 검수는 완료된 상태로 설정하고 다음 단계 진행”.
- `release/CONTENT_REVIEW_STATUS.json`에 현 콘텐츠 해시와 60레슨/1,120문항 범위로 `owner-declared-complete` 기록.
- 실제 검수자·검수일·DBMS·공식 범위 대조 세부 근거는 미제공. 에이전트가 실행하거나 검수한 것으로 쓰지 않았으며 원본 gzip·개별 검수 플래그·공개 배포 승인 게이트는 변경하지 않음.
- `assert-draft-blocked.cjs` 통과: 증거 미연결 상태에서 프로덕션 승인으로 오인하지 않음.
- Google Play의 기존 개발자 계정 접속 성공. SQLD Pass 앱은 목록에 없음.
- 신규 등록 양식에 SQLD Pass / 한국어(ko-KR) / 앱 / 무료 입력.
- 제안 패키지 `com.kasa500000.sqldpass`에 대해 콘솔이 “사용 가능한 패키지 이름” 표시. 등록·예약 또는 영구 식별자 확정은 아님.
- 정책 준수/미국 수출법 동의 체크 및 앱 만들기 제출 전 중지. 법적 동의와 제안 식별자의 최종 확인이 필요함. 다른 앱은 열거나 변경하지 않음.
- App Store Connect는 로그인 화면으로 이동. 사용자 인증이 필요함.
- 브라우저 두 탭을 다음 작업 인계용으로 보존. 새 앱 생성·AAB/IPA 업로드·심사 제출은 아직 수행하지 않음.

## Google Play 앱 생성 — 소유자 승인 후

- 직전 요청의 패키지명·정책 준수 선언·미국 수출법 동의 및 앱 생성에 대한 소유자 “승인”을 받고 실행.
- SQLD Pass / 한국어 / 무료 / `com.kasa500000.sqldpass` 앱 생성 성공.
- Play 앱 ID `4972123172088629069`. 대시보드에서 실제 앱명·생성 상태 확인.
- 개인정보처리방침 URL 저장 후 “변경사항이 저장되었습니다” 확인. 심사 전송은 하지 않음.
- 대시보드가 비공개 테스트 12명/연속 14일 및 현재 0명 조건을 명시함.
- 내부 테스트용 AAB 업로드, 테스트 배포, 프로덕션 신청, iOS 앱 등록은 아직 미완료.
- 상세 등록 식별자: `release/STORE_REGISTRATION.json`. 기존 내부 QA 앱 ID는 유지하며 향후 Play용 빌드는 확정된 Android 패키지를 명시해 생성해야 함.
