...
2026-09-19 소유자가 콘텐츠 검수 완료를 선언해 현재 콘텐츠 해시에 연결하여 `release/CONTENT_REVIEW_STATUS.json`에 기록했다. 검수자·검수일·DBMS 및 공식 범위 대조 근거를 확인 중이다. 원본의 항목별 플래그와 프로덕션 승인파일은 아직 이 선언과 연결되지 않았다. 자동 테스트 통과와 소유자 검수 선언은 구분한다.
