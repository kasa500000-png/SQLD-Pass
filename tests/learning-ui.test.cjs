...
test('locked exam action opens opt-in dialog, not an attempt',async()=>{const {c}=setup();c.route={name:'exams'};await click(c,'exam-SQLD-M05');a.ok(c.dialog);a.equal(c.state.exams.length,0);await c.acceptDialog();a.equal(c.hasAccess('SQLD-M05'),false);a.equal(c.state.exams.length,0);});
